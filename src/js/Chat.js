...
export default class Chat {
  ...
  renderConnectionUser(user) {
    ...
    if (user === this.activeUser) {
      ...
      window.onbeforeunload = async () => {
        ...
        fetch(`https://chat-backend-kd5r.onrender.com/${query}`, {
          method: 'DELETE',
          headers: {
  ...
  async renderChat() {
    ...
    const request = fetch('https://chat-backend-kd5r.onrender.com/index/', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  gettingUsers() {
    const eventSource = new EventSource('https://chat-backend-kd5r.onrender.com/sse');
  ...
  messaging() {
    const ws = new WebSocket('wss://chat-backend-kd5r.onrender.com/ws');
